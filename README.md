...
- **分类布局** - 单列 / 双列布局自由切换，适应不同屏幕宽度
- **拖拽动效** - 卡片拖拽时周围卡片自动让位并回填空位（FLIP 动画）
...
- **颜色样式** - 搜索框、书签卡片、全局字体的颜色和透明度均可独立调整，支持颜色透明度控制
- **内外网切换** - 支持同时配置内网和外网地址，一键切换
- **地址过滤** - 按内网/外网地址筛选显示书签
- **右下角工具栏** - 可拖拽排序、隐藏/显示按钮（设置按钮始终可见），支持一键展开/收回所有分类
...
- 点击右下角展开按钮，显示以下工具：
  - 🎨 主题切换、🔀 内外网、➕ 添加、📂 展开/收回、👤 用户、🔍 网络过滤、⬆ 回到顶部
- **设置** → **布局** → **工具栏按钮管理**：
  - 拖拽调整按钮显示顺序
  - 点击眼睛图标切换按钮可见性
  - 设置按钮始终显示，无法隐藏
...
- 书签卡片支持拖拽排序，拖拽时周围卡片会自动让位并回填空位
- 分类标题支持拖拽调整分类顺序
- 设置面板中的分类排序和工具栏按钮均支持拖拽重排

### 地址过滤

- 点击右下角工具栏的筛选按钮，可按「仅内网」「仅外网」过滤书签
- 过滤状态实时生效，切换时即时更新显示
